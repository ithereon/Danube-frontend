...
const QuoteDetails = ({role, token, userData}) => {
	...
	const endHandler = useCallback(async (id) => {
		setMessage({
			type: 'error',
			body: 'complete feature not yet available'
		});
				...
				<div className='w-full order-1 lg:order-2 lg:w-8/12'>
					{quote && (
						<DetailsCard
							...
							editLink={
								quote.status === 'draft' ? 
									`/${role}/quote/edit?id=${quote.id}`
									: false
							}
							extraActions={
								<>
									{quote.status === 'draft' && quote.work_items.length > 0 && (
										<BorderButton
											clicked={()=>sendHandler(quote.id)}
									...
									{quote.status === 'waiting' && (
										<BorderButton 
											type='button'
											clicked={()=>withdrawHandler(quote.id)}
									...
									{quote.status === 'in_progress' && (
										<BorderButton 
											type='button'
											clicked={()=>endHandler(quote.id)}
											colorStyles='text-indigo-500 border-indigo-500 hover:bg-indigo-500 active:bg-indigo-600 focus:shadow-indigo-600 focus-within:shadow-indigo-600 focus-visible:shadow-indigo-600'
										>
											complete
										</BorderButton>
									)}
