...
const SubscriptionDetails = ({role, token, userData}) => {
	...
	const getData = useCallback(async () => {
		...
		if (res?.data?.id) {
			...
			setInvoice({
				id: res.data.id,
				costs: {
					subtotal: res.data.contract.total_cost,
					discount: res.data.contract.discount,
					discount_amount: res.data.contract.discount_amount,
					subtotal_after_discount: res.data.contract.subtotal_after_discount,
					vat: res.data.contract.vat,
					vat_amount: res.data.contract.vat_amount,
					total_to_pay: res.data.contract.total,
					first_payment_amount: res.data.contract.first_payment_amount,
					balance_to_pay: res.data.contract.total_after_first_payment,
				},
				discount_symbol: res.data.contract.discount_type === 1 ? '%' : '£',
				title: res.data.contract.title,
				description: res.data.contract.description,
				created_at: res.data.created_at,
				status: res.data.status_business.toLowerCase(),
				work_items: res.data.contract.work_items
			});
		}
	}, []);
	...
	const paidHandler = useCallback(async id => {
		const res = await requestWithTokenAsync(
			'post',
			`/invoices/${id}/paid_business/`,
			token
		);
		if (res?.data?.id) {
			setMessage({
				type: 'success',
				body: res.data.status_business === 'PENDING' ? 'invoice first payment paid' : 'invoice paid'
			});
		} else {
			setMessage({
				type: 'error',
		...
		<WithTransition>
          ...
          Subscription details
              ...
              Your Subscription
											...
											return (
												<li className='bg-slate-50 border p-4 rounded-lg' key={item[0]}>
													...
													<div className='flex items-center'>
														<span
															className="w-10 h-10 text-base text-neutral-700 bg-slate-200 inline-flex items-center justify-center rounded-full mr-2">
															<i className="fa-solid fa-user"></i>
														</span>
														<p id="business_user_info"
															className='capitalize text-xs font-medium max-w-full text-center'>
					...
					<div
						className="w-full relative flex flex-col min-w-0 p-6 mt-6 break-words bg-white shadow-xl rounded-lg border-0">
						<h2 className="relative text-slate-600 text-lg font-semibold leading-normal capitalize">
              Business property
							<hr className="mb-2 mt-1 border-b-2 border-emerald-300 w-10"/>
						</h2>
				...
				<div className='w-full order-1 lg:order-2 lg:w-8/12'>
					{invoice && (
						<DetailsCard
							...
							data={Boolean(invoice.id)}
							extraActions={
								<>
									{invoice.status === 'open' && (
										<BorderButton
											clicked={() => paidHandler(invoice.id)}
                      ...
                      mark as paid
									...
									{/* <BorderButton
										href={`/${role}/contract/pdf?id=${contract.id}`}
										target="_blank"
										type='link'
										// clicked={()=>router.push(`/${role}/contract/pdf?id=${contract.id}`)}
										colorStyles='text-blue-600 border-blue-600 hover:bg-blue-600 active:bg-blue-700 focus:shadow-blue-700 focus-within:shadow-blue-700 focus-visible:shadow-blue-700'
									>
										<i className='fas fa-file-lines font-normal mr-1' /> view PDF
									</BorderButton> */}
								</>
							}
						>
...
SubscriptionDetails.auth = true;
SubscriptionDetails.layout = Admin;
