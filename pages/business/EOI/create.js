...
const CreateEOI = ({userData, token, role}) => {
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			if(res?.data?.id){
				setMessage({
					body: `You have expressed interest in RFQ with ID of ${res.data.id}`,
					type: 'success'
				});
				setTimeout(() => {
				...
				<div className="w-full lg:w-4/12 px-4 mb-4 lg:mb-0">
					<div className='flex flex-col justify-between'>
						...
						{/* <div className='mt-3 mb-6'>
							<Feedback state="information" message="Your contact information won't be shared to the customer" />
						</div> */}
					</div>
				</div>
