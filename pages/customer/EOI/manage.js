		...
		<div className='md:container mx-auto pb-8 mb-6 flex flex-wrap items-stretch'>
			<WithTransition>
				<StyledModal
					...
					aria-labelledby="decline EOI"
				>
					<p className='text-neutral-700'>
						{`Are you sure you want to decline EOI with ID of ${open.id}?`}
					</p>
									...
									<ListItem
										...
										comment={
											<span className='line-clamp-1'>
												<i className='fas fa-comment-dots rotate-y-180 pl-1 text-blue-500' />
												{item.comment}
											</span>}
										...
										actions={
											<ul>
												...
												{(item.status !== 'declined' && item.status !== 'accepted') && (
													<li>
														<Link href={`/${role}/quote/create?id=${item.id}`}>
