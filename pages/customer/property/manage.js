...
const ManageProperties = ({token, role}) => {
	...
	const fetchData = async (keyword) => {
		const res = await requestWithTokenAsync(
			'get',
			`/properties/?search=${keyword ? keyword : ''}&limit=${perPageLimit}&offset=${(Number(p)-1)*perPageLimit}`,
			token
		);
		if(res?.data?.results && res.data.results.length > 0){
			const modifiedData = res.data.results.map(item=>({
				title: item.address_1,
				subTitle: item.address_2 ? item.address_2 : item.town,
				property: {
					...
					<div className="relative flex flex-col min-w-0 break-words w-full h-full shadow-lg rounded-lg bg-slate-50 border-0">
						<div className="flex-auto">
							...
							>
								...
								{/* <StyledTable
									rows={['address line 1', 'postcode', 'property ID', 'view', 'edit', 'delete']}
									data={data}
								/> */}
							</ListContainer>
						</div>
...
ManageProperties.auth = true;
ManageProperties.layout = Admin;