...
const EditRFQ = ({userData, token, role}) => {
	const router = useRouter();
	const {id, itemsMode} = router.query;
	...
	return (
		<MainEdit token={token} profile={userData} id={id} />
	);
...
EditRFQ.auth = true;
EditRFQ.layout = Admin;


const MainEdit = ({token, profile, id}) => {
	const router = useRouter();
	const [message, setMessage] = useState();
	const [selectedProperty, setSelectedProperty] = useState();
	...
	useEffect(() => {
		const getRFQ = async () => {
			...
			if(res?.data?.id){
				const allowedStatuses = ['draft', 'saved', 'open', 'private'];
				if(allowedStatuses.includes(res.data.status.toLowerCase())){
					const propert = {
						address_1: res.data.property.address_1,
						address_2: res.data.property.address_2,
						town: res.data.property.town,
						city: res.data.property.city,
						county: res.data.property.county,
						postcode: res.data.property.postcode
					};
					Object.keys(propert).forEach(key => {
						if (propert[key] === null || propert[key] === '') {
							delete propert[key];
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			if(res?.data?.id){
				setMessage({
					body: `RFQ with ID of ${res.data.id} has been updated`,
					type: 'success'
				});
				router.push(`?id=${id}&itemsMode=true`);
			}else{
				setMessage({
					body: 'Something went wrong',
					type: 'error',
